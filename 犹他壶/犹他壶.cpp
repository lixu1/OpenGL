...
float vertices[][3]={{1.4 , 0.0 , 2.4}, {1.4 , -0.784 , 2.4},
...
void display(void)
{
	...
	for(k=0;k<32;k++)
	{
		...
		for(j=0;j<=4;j++)
		{
			...
			for(i=0;i<=20;i++)
				glEvalCoord2f((GLfloat)i/20.0,(GLfloat)j/4.0);
			glEnd();
			glBegin(GL_LINE_STRIP);
			for(i=0;i<=20;i++)
				glEvalCoord2f((GLfloat)j/4.0,(GLfloat)i/20.0);
			glEnd();
		}
...
void myReshape(int w,int h)
{
	...
	if(w<=h)
		...
		glOrtho(-4.0,4.0,-4.0*(GLfloat)w/(GLfloat)h,4.0*(GLfloat)w/(GLfloat)h,-20.0,20.0);
...
void main(int argc,char* argv[])
{
	...
	glutCreateWindow("11211126_ÀîÐñ_ÓÌËûºø");
	myinit();
